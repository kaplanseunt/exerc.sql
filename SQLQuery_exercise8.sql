...
--my code
	...
	--,pt.patient_id,pt.event_name,ec.category 
	from
	physician_speciality ps join patient_treatment pt on ps.physician_id = pt.physician_id
	join  event_category ec on pt.event_name = ec.event_name
	where ec.category = 'Procedure' and
	pt.physician_id not in (
						select pt2.physician_id from patient_treatment pt2  join event_category ec2   on ec2.event_name = pt2.event_name
						where ec2.category in ('Prescription')
	)
